...
#include "Include/SonicR_StateMachine.mqh"  // State machine
#include "Include/SonicR_PropSettings.mqh"  // PropFirm settings

// Các thành phần phân tích thị trường
#include "Include/SonicR_Filters.mqh"  // Session, News và Market Regime filters
#include "Include/SonicR_PVSRA.mqh"  // PVSRA analysis
#include "Include/SonicR_SimplePVSRA.mqh"  // Simple PVSRA

// Các thành phần chiến lược
#include "Include/SonicR_Core.mqh"  // Core strategy
#include "Include/SonicR_SR.mqh"  // Support/Resistance system

// Các thành phần quản lý giao dịch
#include "Include/SonicR_RiskManager.mqh"  // Risk management
#include "Include/SonicR_AdaptiveFilters.mqh"  // Adaptive filters
#include "Include/SonicR_EntryManager.mqh"  // Entry management
#include "Include/SonicR_ExitManager.mqh"  // Exit management

// Giao diện người dùng
#include "Include/SonicR_Dashboard.mqh"  // Dashboard
...
int OnInit()
{
    ...
    g_logger.Info("Initializing SonicR PropFirm EA");
    ...
    g_stateMachine.SetLogger(g_logger);
    
    // Khởi tạo PropSettings
    ...
    g_propSettings.SetLogger(g_logger);
    
    // Nếu đặt prop firm là custom, thiết lập các giá trị tùy chỉnh
    ...
    g_trade.LogLevel(LOG_LEVEL_ERRORS); // Chỉ ghi log lỗi từ đối tượng trade
    ...
    g_sonicCore.SetLogger(g_logger);
    ...
    if(!InitializeFilters()) {
        g_logger.Error("Failed to initialize filters");
        return INIT_FAILED;
    }
    ...
    g_riskManager.SetRecoveryMode(UseRecoveryMode, RecoveryReduceFactor);
    
    // Khởi tạo Adaptive Filters
    ...
    g_entryManager.SetLogger(g_logger);
    g_entryManager.SetSRSystem(g_srSystem);
    ...
    g_exitManager.SetLogger(g_logger);
    ...
    // Thành công, chuyển sang trạng thái SCANNING
    g_initialized = true;
    g_stateMachine.TransitionTo(STATE_SCANNING, "Initialization complete");
    
    g_logger.Info("SonicR PropFirm EA v2.00 initialized successfully");
...
//+------------------------------------------------------------------+
//| Initialize market analysis filters                                |
//+------------------------------------------------------------------+
bool InitializeFilters()
{
    ...
    g_sessionFilter.SetLogger(g_logger);
    ...
    g_newsFilter.SetLogger(g_logger);
    ...
    g_marketRegimeFilter.SetLogger(g_logger);
...
bool InitializeDashboard()
{
    ...
    g_dashboard.SetDependencies(
        g_sonicCore, 
        g_riskManager, 
        g_stateMachine, 
        g_propSettings
    );
    g_dashboard.SetAdaptiveFilters(g_adaptiveFilters);
    g_dashboard.SetPVSRA(g_pvsra);
