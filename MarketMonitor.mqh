...
CMarketMonitor::CMarketMonitor(void)
{
   ...
   // Initialize logger
   m_Logger = CLogger("MarketMonitor");
...
//+------------------------------------------------------------------+
//| Cập nhật dữ liệu thị trường và phân tích                         |
//+------------------------------------------------------------------+
bool CMarketMonitor::Update()
{
   if (!m_IsInitialized) {
      m_Logger.LogError("Market Monitor chưa được khởi tạo");
      return false;
   }
   
   // Biến static cho việc theo dõi
   static datetime lastBarTime = 0;        // Thời gian của thanh gần nhất
   static datetime lastFullUpdate = 0;     // Thời gian cập nhật đầy đủ cuối cùng
   static double lastPrice = 0;            // Giá cuối cùng
   ...
   // Xác định xem có cần cập nhật đầy đủ không
   bool isNewBar = (currentBarTime > lastBarTime);
   bool significantMove = false;
   
   // Kiểm tra nếu giá đã di chuyển đáng kể (0.2 ATR)
   if (m_Cache.atr_entry > 0) {
      significantMove = (MathAbs(currentPrice - lastPrice) > m_Cache.atr_entry * 0.2);
   }
   
   // Kiểm tra nếu đã đủ thời gian kể từ lần cập nhật đầy đủ cuối cùng (15 giây)
   bool timeInterval = (currentTime - lastFullUpdate > 15);
   
   // Cập nhật đầy đủ nếu (bar mới HOẶC giá thay đổi đáng kể HOẶC đủ thời gian qua)
   if (isNewBar || significantMove || timeInterval) {
      // Lưu các giá trị hiện tại
      lastBarTime = currentBarTime;
      lastPrice = currentPrice;
      lastFullUpdate = currentTime;
      
      // Đặt cờ new bar để các hàm khác biết
      m_IsNewBar = isNewBar;
      
      // Cập nhật đầy đủ các chỉ báo
      if (!UpdateIndicatorData()) {
         m_Logger.LogWarning("Không thể cập nhật dữ liệu chỉ báo");
         return false;
      }
      ...
      if (isNewBar) {
         if (!UpdateMarketState()) {
            m_Logger.LogWarning("Không thể cập nhật trạng thái thị trường");
            return false;
         }
         
         // Log trạng thái thị trường trên bar mới để debug
         if (m_Logger.IsDebugEnabled()) {
            string regime = RegimeToString(m_MarketRegime);
            string trend = m_IsTrendUp ? "Lên" : (m_IsTrendDown ? "Xuống" : "Trung tính");
      ...
      // Chỉ cập nhật thông tin giá cơ bản
...
//+------------------------------------------------------------------+
//| LightUpdate - Cập nhật nhẹ chỉ lấy thông tin giá hiện tại        |
//+------------------------------------------------------------------+
bool CMarketMonitor::LightUpdate()
{
   if (!m_IsInitialized) {
      m_Logger.LogError("Market Monitor chưa được khởi tạo");
      return false;
   ...
   // Chỉ cập nhật thông tin giá hiện tại, không cập nhật chỉ báo
   m_Cache.close = SymbolInfoDouble(m_Symbol, SYMBOL_BID);
...
bool CMarketMonitor::DetectFibonacciPullback(bool isUptrend, double &entryLevels[], double &qualityScore)
{
   ...
   qualityScore = 0;
   ArrayResize(entryLevels, 5); // 5 mức Fibonacci tiềm năng
   ArrayInitialize(entryLevels, 0);
   
   // Lấy ATR để đo lường tương đối
   double atr = m_Cache.atr_entry;
   if (atr <= 0) {
      m_Logger.LogDebug("ATR không hợp lệ cho phân tích Fibonacci");
      return false;
   }
   
   // Định nghĩa các mức Fibonacci
   double FIB_LEVELS[] = {0.236, 0.382, 0.5, 0.618, 0.786};
   int FIB_COUNT = ArraySize(FIB_LEVELS);
   
   // Lấy dữ liệu giá với kiểm tra an toàn
   ...
   int MAX_LOOKBACK = 100; // Tối đa số thanh để phân tích
   
   if (!SafeCopyRates(m_Symbol, m_EntryTimeframe, 0, MAX_LOOKBACK, open, high, low, close)) {
      m_Logger.LogError("Không thể sao chép dữ liệu giá cho phân tích Fibonacci");
      return false;
   }
   
   // Tìm điểm swing quan trọng
   double swingThreshold = atr * 0.5; // Chuyển động tối thiểu để xác định swing
   
   // Mảng để lưu các điểm swing
   double swingPrices[];
   int swingBars[];
   bool swingTypes[]; // true = high, false = low
   int swingCount = 0;
   
   // Tìm tất cả điểm swing (zigzag) trong phạm vi lookback
   for (int i = 2; i < MAX_LOOKBACK - 2; i++) {
      // Tìm đỉnh cục bộ
      if (high[i] > high[i-1] && high[i] > high[i-2] && 
          high[i] > high[i+1] && high[i] > high[i+2]) {
         
         // Chỉ chấp nhận swing nếu biên độ đủ lớn so với swing trước
   ...
   // Kiểm tra nếu không tìm thấy đủ điểm swing
   if (swingCount < 2) {
      m_Logger.LogDebug("Không đủ điểm swing cho phân tích Fibonacci");
      return false;
   }
   
   // Tìm cặp swing phù hợp cho tính toán Fibonacci
   int swingStartIdx = -1; 
   int swingEndIdx = -1;
   
   for (int i = 0; i < swingCount - 1; i++) {
      // Đối với uptrend, cần tổ hợp High->Low (đỉnh->đáy)
      if (isUptrend && swingTypes[i] == true && swingTypes[i+1] == false) {
         swingStartIdx = i;
         swingEndIdx = i+1;
         break;
      }
      // Đối với downtrend, cần tổ hợp Low->High (đáy->đỉnh)
   ...
   // Kiểm tra nếu tìm thấy cặp swing phù hợp
   if (swingStartIdx < 0 || swingEndIdx < 0) {
      m_Logger.LogDebug("Không tìm thấy cặp swing phù hợp cho phân tích Fibonacci");
      return false;
   }
   
   // Lấy giá trị và vị trí của điểm swing
   ...
   // Kiểm tra tuổi của pullback
   if (barEnd > 20) { // MAX_RETRACEMENT_AGE
      m_Logger.LogDebug(StringFormat("Pullback quá cũ: %d bars", barEnd));
      return false;
   }
   
   // Tính kích thước sóng impulse
   double impulseSize = MathAbs(swingStart - swingEnd);
   
   // Kiểm tra kích thước impulse
   if (impulseSize < atr * m_FibMinImpulseSize) {
      m_Logger.LogDebug(StringFormat("Sóng impulse quá nhỏ: %.2f ATR", impulseSize/atr));
      return false;
   }
   ...
   for (int i = 0; i < FIB_COUNT; i++) {
      if (isUptrend) {
         // Đối với uptrend: Tính từ đỉnh xuống đáy, sau đó retracement lên
         entryLevels[i] = swingEnd + impulseSize * FIB_LEVELS[i];
      } else {
         // Đối với downtrend: Tính từ đáy lên đỉnh, sau đó retracement xuống
         entryLevels[i] = swingEnd - impulseSize * FIB_LEVELS[i];
      }
   }
   
   // Kiểm tra nếu giá hiện tại gần mức Fibonacci nào
   ...
   // Chuyển đổi khoảng cách thành tỷ lệ ATR
   double distanceInATR = minDistance / atr;
   
   // Kiểm tra nếu giá không gần bất kỳ mức Fibonacci nào
   if (distanceInATR > m_FibonacciNearThreshold || closestFibIdx < 0) {
      m_Logger.LogDebug(StringFormat("Giá hiện tại không gần mức Fibonacci nào (%.2f ATR)", distanceInATR));
      return false;
   }
   
   // Phân tích chất lượng giao dịch Fibonacci
   
   // Điểm chất lượng cơ bản
   qualityScore = 0.6;
   
   // Thưởng cho vùng vàng (0.382-0.618)
   if (closestFibIdx >= 1 && closestFibIdx <= 3) {
      qualityScore += m_GoldenZoneBonus;
   }
   
   // Thưởng cho độ chính xác
   qualityScore += (1.0 - (distanceInATR / m_FibonacciNearThreshold)) * 0.1;
   
   // Thưởng nếu retracement Fib phù hợp với khung thởi gian cao hơn
   if (m_UseMultiTimeframe) {
      bool higherTFUptrend = (m_Cache.ema_fast_higher > m_Cache.ema_trend_higher);
      if (isUptrend == higherTFUptrend) {
         qualityScore += 0.1; // Thưởng cho sự phù hợp
      }
   }
   ...
   m_Logger.LogInfo(StringFormat("Phát hiện Fibonacci %.1f%% Pullback, Chất lượng: %.2f", 
